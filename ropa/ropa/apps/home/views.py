...
from ropa.apps.ventas.models import Ropa
from ropa.apps.home.forms import contact_form
from django.core.mail import EmailMultiAlternatives
from ropa.apps.home.forms import add_product_form, login_form
...
def ropas_view(request, pagina):
	lista_prod = Ropa.objects.filter(status = True)
	...
	ctx  = {'ropas':productos}
	return render_to_response('home/ropa.html', ctx, context_instance =RequestContext(request))
...
def contacto_view(request):
	...
	if request.method == "POST":
		...
		if formulario.is_valid():
			...
			to_admin = ' richard-10@misena.edu.co'
...
def single_ropa_view(request, id_ropa):
	prod = Ropa.objects.get(id = id_ropa)
	ctx  = {'ropa':prod}
	return render_to_response('home/single_ropa.html',ctx,context_instance = RequestContext(request))
